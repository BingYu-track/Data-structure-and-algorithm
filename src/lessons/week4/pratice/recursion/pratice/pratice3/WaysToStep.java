...
public class WaysToStep {
    ...
    */
    ...
    //执行结果: 79 ms	 143.7 MB，使用备忘录很耗性能，如果不使用备忘录，时间复杂度为O(3^n)
    /*
      TODO 问: 为什么这个题目使用备忘录会很耗时呢？按照递归树画出来，时间复杂度也是O(n)的呀
           答：这是和题目的测试的数据规模有关，这次n的范围在[1, 1000000]之间，相当于要调用
           1百万次函数，然而调用函数也是要花时间的，前2题范围都是在[1,100]之间，因此数据规模
           不大，调用函数的次数也少，但是调用次数一大增大，调用函数的耗时就会体现出来，因此没有
           后面迭代法快，迭代法同样是O(n)，但是并不会调用函数，因此耗时自然要少的多！
     */
