      ...
      题型7: 按照遍历结果反向构造二叉树
       ...
       TODO 关于前序、后序构建二叉树的情况，需要拿出来单独说一下
        解题前置知识--     前序数组排列特点:根、左子树所有元素、右子树所有元素
                  ...
                  2.在后序中，如果 1 号位置在 2 号位置之间右边并且二者之间有元素，说明之间的元素都属于1号的右子树，2号位置
                    及以前的元素都属于1号的左子树，如果递归下去都是这样，就是唯一的；如果 1 号和 2 号位置之间没有元素，这时
                    就没有办法判断它是属于左子树属于右子树，二叉树也就不唯一了。
                下面对于 2 个具体的例子给出图示：
        ...
        一、二叉树的存储
          ...
          3.节点个数为n的二叉树，高度是多少?
            TODO: 首先要明确二叉树"高度"和"深度"的定义，因为关于高度和深度，许多书籍与教材的说法大不一致，
                树中的某节点的深度是它到根节点的路径上的边的条数。
                树中某节点的高度是指从该节点到叶子节点的最长简单路径边的条数。
                树的高度等于所有节点的最大深度(即树的深度)。

            a.最大高度: 为n，即没有分叉相当于链表
    ...
    */
    ...
    public class Node {
        ...
        c.第三种情况: 要删除的节点有两个子节点。需要找到这个节点的右子树中的最小节点(即在右子树里不断往左寻找)，把它替换到要
           ...
           肯定没有左子节点，只可能是有一个右子节点，或者，没有子节点所以我们此时可以应用上面两天规则来删除这个最小节点。
        */
