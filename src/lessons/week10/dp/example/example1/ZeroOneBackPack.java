...
public class ZeroOneBackPack {
    ...
    /* 定义一个函数f(a,b) a表示处理的几个石头的下标，b表示放入背包里石头的总重量，以{2,2,4,6,3}数据为例
       ...
       4 | 1 | 0 | 1 | 1 | 1 | 1 | 1 | 1 | 1 | 1 |  下标为4的物品重量为3
        ...
        TODO: 注意这个行不是向背包放入物品的个数，而是考察物品的个数
     */
...
//        }
        ...
        //TODO：这里先遍历背包容量，再遍历物品
        for (int j = w;j >= 0;j--) { //遍历背包容量
            for (int i = 1;i < n;i++) { //遍历物品
                if (dp[i-1][j] || (j-weight[i]>=0 && dp[i-1][j-weight[i]])) {
                    if (dp[i-1][j]) {
                        System.out.print("(" + i + "," + j + ")->(" + (i-1) +"," + j + ") "); //打印dp
    ...
    /*
     先遍历物品，后遍历背包的dp打印结果(从前往后，一行行考察的):
    第1行考察:(1,0)->(0,0) (1,2)->(0,2) (1,2)->(0,0) (1,4)->(0,2)   TODO: 可以看到每行都是放入同一种物品
    第2行考察:(2,0)->(1,0) (2,2)->(1,2) (2,4)->(1,4) (2,4)->(1,0) (2,6)->(1,2) (2,8)->(1,4)
    第3行考察:(3,0)->(2,0) (3,2)->(2,2) (3,4)->(2,4) (3,6)->(2,6) (3,6)->(2,0) (3,8)->(2,8) (3,8)->(2,2)
    第4行考察:(4,0)->(3,0) (4,2)->(3,2) (4,3)->(3,0) (4,4)->(3,4) (4,5)->(3,2) (4,6)->(3,6) (4,7)->(3,4) (4,8)->(3,8) (4,9)->(3,6)

    先遍历背包，后遍历物品的dp打印结果(从上到下，一列列考察的):
     第0列考察:   (1,0)->(0,0) (2,0)->(1,0) (3,0)->(2,0) (4,0)->(3,0)
     第1列考察:  没有满足条件的 TODO: 可以看到每行放入涉及到不同种类的物品，
     第2列考察:   (1,2)--不放物品2-->(0,2)  (1,2)--放入物品2-->(0,0) (2,2)--不放物品3-->(1,2) (3,2)--不放物品6-->(2,2) (4,2)--不放物品3-->(3,2)
     第3列考察:   (4,3)->(3,0)
     第4列考察:   (1,4)--放入物品2-->(0,2) (2,4)--不放-->(1,4) (2,4)--放入物品4-->(1,0) (3,4)->(2,4) (4,4)->(3,4)
     第5列考察:   (4,5)--放入物品3-->(3,2)
     第6列考察:   (2,6)->(1,2) (3,6)->(2,6) (3,6)->(2,0) (4,6)->(3,6)
     第7列考察:   (4,7)->(3,4)
     第8列考察:   (2,8)->(1,4) (3,8)->(2,8) (3,8)->(2,2) (4,8)->(3,8)
     第9列考察:   (4,9)->(3,6)

    TODO: 从上面例子，可以看到dp的最终结果都是一样的，为什么这个二维数组dp的两层for循环遍历的顺序就可以随意颠倒?(重要)
       因为，这里的二维数组dp都是从前往后进行遍历的，具体回答和分析见example6
   */
}
