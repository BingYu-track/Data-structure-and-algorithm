...
//        int[] nums = {-1};
    ...
    /*
     ...
     以-2,1,-3,4,-1,2,1,-5,4为例，因为是取sum的最大值，因此在两指针之间的sum是一个负数，想要增大只能后面是正数才行，因此不如让start抛弃当前负数，继续遍历
     下一个数，只要下一个数不为负数，这一个数就比两个数的和大，对应上面例子就是当satrt=0,end=1时，sum是个负数，因此start往后移动一位抛弃-2，从1
