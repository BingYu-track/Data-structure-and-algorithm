...
public class DiGraph {
  ...
  /* TODO： c.检测图中是否有环
    ...
    /*
     思路2: 使用dfs检测环，先从入度为0的节点开始dfs，每个个节点在初始状态为白色，访问该节点时染成红色，并在退出时染成黑色。如果DFS遍历中发现了
       红色节点，则说明有环。先选取任意一个未访问过的点开始遍历
   TODO: 我之前写的方法不适用于非连通图，因为我只找一个入度为0的点，然后进行dfs遍历，如果是非连通图的话，会有多个入度为0的点，
    这样可能就错过遍历另一个子图了，如果刚好这个子图里有环，那这就会判断错误;还有一种可能就是子图里形成了一个环，子图里所有的顶点入度
    都不为0.我这种dfs根本就无法遍历到这个子图上，也会出错! 所以在使用dfs时需要先遍历多余节点，对每个未访问的节点进行DFS，然后再
    */
